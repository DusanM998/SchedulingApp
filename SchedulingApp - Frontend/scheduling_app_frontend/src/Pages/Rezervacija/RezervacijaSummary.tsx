...
function RezervacijaSummary() {
  ...
  const handleTerminSelection = (
    ...
    setSelectedTermini((prev) => {
      ...
      if (noviTermini.length === 0) {
        ...
        delete noviState[sportskiObjekatId]; // Alternativa: noviState[sportskiObjekatId] = []
        return noviState;
      }

      // Ako postoji bar jedan termin, računamo novu cenu
  ...
  const handleBrojUcesnika = (
    ...
    console.log("Slanje podataka za azuriranje:", {
      stavkaKorpe,
      novaKolicina,
    });
    dispatch(
  ...
  const handleExpandCard = (sportskiObjekatId: number) => {
    ...
    setExpandedCard(
      expandedCard === sportskiObjekatId ? null : sportskiObjekatId
    );
  ...
  const handleConfirmSelection = (
    ...
    console.log(
      "Tip podataka terminIds:",
      Array.isArray(terminiIds),
      typeof terminiIds[0]
    );
    ...
    console.log("Azuriranje termina u korpi", {
      sportskiObjekatId,
      terminiIds,
    });
  };

  // Azurira cenu svaki put kada se promene selektovani termini ili kolicina
  useEffect(() => {
  ...
  // Azurira Redux store kada se selektovani termini promene
  useEffect(() => {
    if (
    ...
    <div className="container p-4 m-2">
      ...
      {shoppingCartStore.length === 0 ? (
        ...
        shoppingCartStore.map(
          (stavkaKorpe: stavkaKorpeModel, index: number) => (
            <div
              key={index}
                    ...
                    <div>
                      <div className="d-flex flex-wrap justify-content-center">
                        {termini.map((termin: terminModel) => {
                          const isSelected = selectedTermini[
                            stavkaKorpe.sportskiObjekat!.sportskiObjekatId
                          ]?.some((t) => t.terminId === termin.terminId);
